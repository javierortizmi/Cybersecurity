...
def prime_test(n: int, k: int) -> bool:
...
def generate_prime_number(bit_length: int) -> int:
    ...
    while True:
        ...
        # with k rounds, prime test gives false positive with probability (1/4)^k = 1/(2^2k)
        k = 64
        if prime_test(candidate_prime, k):
...
def rsa_encrypt(plaintext: str, e: int, n: int) -> int:
    plaintext = plaintext.encode()
    p_int = int.from_bytes(plaintext, "big")
    return pow(p_int, e, n)


def rsa_decrypt(ciphertext: int, d: int, n: int) -> str:
    p_int = pow(ciphertext, d, n)
    return p_int.to_bytes((p_int.bit_length() + 7) // 8, 'big').decode()